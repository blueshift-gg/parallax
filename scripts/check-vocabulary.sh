#!/usr/bin/env bash
# Tripwire for vocabulary that died in an API round: stale grammar in docs or
# code is a bug. Extend the pattern list whenever a rename retires a name.
set -euo pipefail
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
dead='succeeds\(|fails\(|fails_with|SucceededTransaction|FailedTransaction|TestBuilder|TestOptions|&mut Test\b|Test::builder|Test\.open|has_lamports|has_tokens|has_supply|cu_at_most|owned_by|is_closed\(|hasLamports|hasTokens|hasSupply|cuAtMost|ownedBy|isClosed\(|CuBudget|Lamports::|Tokens::|Supply::|Owner::eq|Changes::|Changes\.|::state\(|with::<|Assert::|\.sendAll|sendWith|\.send\(|send_all|test\.send|simulate_all|simulateAll'
if grep -rnE "$dead" "$root/README.md" "$root/docs" "$root/.claude/skills" "$root/derive/README.md" 2>/dev/null; then
  echo "stale vocabulary found" >&2
  exit 1
fi
echo "vocabulary clean"
